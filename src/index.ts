...
function solvePermute(
    ...
    const combinedConstraints = constraints.filter((constraint) => exclude.indexOf(constraint)).reduce((a, b) => {
        const abSyms = new Set<symbol>();
        a.syms.forEach(abSyms.add);
        b.syms.forEach(abSyms.add);
    ...
    // console.log("combinedConstraints", combinedConstraints.toString());
...
interface IResult {
    fx: number;
    dfdx: Map<symbol, number>;
    dfdxx: Map<symbol, number>;
